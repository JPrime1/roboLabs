...
class CollectLidar(Node):
    ...
    def _scan_callback(self, msg):
        ...
        yPos = np.zeros(len(ranges), dtype=np.int16)
        xPos = np.zeros(len(ranges), dtype=np.int16)
        for i in range(len(ranges)):
             y = ranges[i]*zoom*math.sin(angle_increment*i) + centery
             x = ranges[i]*zoom*math.cos(angle_increment*i) + centerx
             self._map[int(y)][int(x)] = 255
